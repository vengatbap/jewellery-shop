import { CountryRepository } from '../repositories/country.repository';
import { cacheManager } from '@auric-one/cache';
import { EventPublisher } from '../events/event.publisher';
import { CatalogQueryParams } from '../repositories/query.helper';
import { countries } from '@auric-one/database';

export class CountryService {
    private repo = new CountryRepository();

    private getCacheKey(suffix: string) {
        return `catalog:global:v1:countries:${suffix}`;
    }

    private async invalidateCache() {
        await cacheManager.delByPrefix(`catalog:global:v1:countries`);
    }

    async getById(id: string) {
        const cacheKey = this.getCacheKey(`id:${id}`);
        const cached = await cacheManager.get<any>(cacheKey);
        if (cached) return cached;

        const country = await this.repo.findById(id);
        if (country) {
            await cacheManager.set(cacheKey, country, 300);
        }
        return country;
    }

    async getMany(params: CatalogQueryParams) {
        const cacheKey = this.getCacheKey(`list:${JSON.stringify(params)}`);
        const cached = await cacheManager.get<{ data: any[]; total: number }>(cacheKey);
        if (cached) return cached;

        const data = await this.repo.findMany(params);
        const total = await this.repo.count(params);
        const result = { data, total };

        await cacheManager.set(cacheKey, result, 300);
        return result;
    }

    async create(data: typeof countries.$inferInsert, userId?: string) {
        const country = await this.repo.create(data);
        await this.invalidateCache();
        await EventPublisher.publishCatalogChanged({
            entity: 'countries',
            action: 'CREATE',
            payload: country,
            userId
        });
        return country;
    }

    async update(id: string, data: Partial<typeof countries.$inferInsert>, version: number, userId?: string) {
        const country = await this.repo.update(id, data, version);
        if (!country) return null;

        await this.invalidateCache();
        await EventPublisher.publishCatalogChanged({
            entity: 'countries',
            action: 'UPDATE',
            payload: country,
            userId
        });
        return country;
    }

    async delete(id: string, userId?: string) {
        const country = await this.repo.delete(id);
        if (!country) return null;

        await this.invalidateCache();
        await EventPublisher.publishCatalogChanged({
            entity: 'countries',
            action: 'DELETE',
            payload: country,
            userId
        });
        return country;
    }
}
